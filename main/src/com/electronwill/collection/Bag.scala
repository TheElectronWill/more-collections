...
/**
 * A resizeable unordered collection that allows duplicate.
 * Calling `remove(i)` moves the last element to index i and is therefore O(1).
 *
 * @author TheElectronWill
 */
abstract class Bag[A] extends mutable.Iterable[A] with Compactable {
  /**
   * Gets an element from the bag. O(1)
  ...
  /**
   * Removes an element from the bag. O(1)
   * @param i the element's index, the first element being at index 0
   */
  ...
  /**
   * Adds an element to the bag. Amortized O(1)
